...
class JwtAuth {
  ...
  */
  register(request, response, next) {
    const validation = validator.validator(request);
    if (validation) {
    ...
    authRepository.registerUser(userInfo).then(async (data) => {
        if (data === 1) {
            return response.json({ 
        ...
        console.log("REGISTER EMAIL======>>" + JSON.stringify(emailResponse));
        return response.json({ 
